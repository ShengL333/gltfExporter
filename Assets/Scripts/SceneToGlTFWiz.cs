...
namespace Uinty2glTF
{
    ...
    public class SceneToGlTFWiz : MonoBehaviour
    {
        ...
        public void ExportCoroutine(string path, Preset presetAsset, bool buildGlb, bool exportPBRMaterials, bool exportAnimation, bool doConvertImages, Action<bool, string> callBack)
        {
            StartCoroutine(Export(path, presetAsset, buildGlb, exportPBRMaterials, exportAnimation, doConvertImages, callBack));
        }
        ...
        public IEnumerator Export(string path, Preset presetAsset, bool buildGlb, bool exportPBRMaterials, bool exportAnimation, bool doConvertImages, Action<bool, string> callBack)
        {
            ...
            foreach (Transform tr in trs)
            {
                ...
                if (m != null)
                {
                    ...
                    for (var i = 0; i < smCount; ++i)
                    {
                        ...
                        #region technique and materials
                        ...
                        if (tr.GetComponent<SkinnedMeshRenderer>() != null)
                        {
                            ...
                            {
                                int bsCount = m.blendShapeCount;
                                for (int j = 0; j < bsCount; j++)
                                {
                                    string bsName = m.GetBlendShapeName(j);

                                    GlTF_Accessor bspositionAccessor = new GlTF_Accessor(bsName, GlTF_Accessor.Type.VEC3, GlTF_Accessor.ComponentType.FLOAT);
                                    ...
                                    mesh.weight.Add(skin.GetBlendShapeWeight(j)/100);
                                    blendShape.blendShapeIndex = j;
                                    blendShape.morphFrame = m.GetBlendShapeFrameCount(j) - 1; 
                               
                                    blendShape.name = bsName;
                                    blendShape.positionAccessor = bspositionAccessor;
                                    primitive.blendShapes.Add(blendShape);
                                    //names.Add(blendShape.name);
                                    //List<float> tempweight = mesh.weight;
                                    ...
                                    //if (!File.Exists("E:/Unity/GLBEXproter/glTFtest\\Test\\items.csv"))
                                    //{
                                    //    File.Create("E:/Unity/GLBEXproter/glTFtest\\Test\\items.csv");
                                    //}
                                    ////open the file with a stream writer for editing
                                    //using (var streamWriter = new StreamWriter("E:/Unity/GLBEXproter/glTFtest\\Test\\items.csv"))
                                    //{
                    ...
                    mesh.at = attributes;//attributes.Add(attributes);
                ...
                else
                {
                    ...
                    if (tr.localPosition != Vector3.zero)
                    {
                        ...
                        Vector3 temp = new Vector3(node.translation.items[0], node.translation.items[1], node.translation.items[2]);
                        //node.translation = new GlTF_Translation(temp);
                    }
                    ...
                    if (tr.localRotation != Quaternion.identity)
                    {
                        ...
                        Quaternion temp = new Quaternion(node.rotation.items[0], node.rotation.items[1], node.rotation.items[2], node.rotation.items[3]);
                        //node.rotation = new GlTF_Rotation(temp);
                    }
                ...
                if (exportAnimation && skinMesh != null && skinMesh.enabled && checkSkinValidity(skinMesh, trs) && skinMesh.rootBone != null)
                {
                    ...
                    skin.name = GlTF_Writer.cleanNonAlphanumeric(skinMesh.rootBone.name);
            ...
            GlTF_Writer.nodes[0].matrix = null;
            GlTF_Writer.nodes[0].translation = new GlTF_Translation(new Vector3(0, 0, 0));
            //GlTF_Writer.nodes[0].rotation = new GlTF_Rotation(new Quaternion(0, 1, 0, 0));
            if (GlTF_Writer.meshes.Count == 0)
            {
                //Debug.Log("No visible objects have been exported. Aboring export");
                //yield return false;
                if (callBack != null) callBack.Invoke(false, "导出失败，没有可供导出的网格对象！");
                yield break;
            }
            ...
            List<GlTF_Accessor> chekc = GlTF_Writer.accessors;
            List<GlTF_Node> nodes = GlTF_Writer.nodes;
            int count = GlTF_Writer.accessors.Count;

            if (callBack != null) callBack.Invoke(true, "导出成功。");
            yield return true;
        ...
        private bool handleTransparency(ref Material mat, ref GlTF_Material material)
        {
            ...
            if (mat.GetFloat("_AlphaCutoff") != 0f)
            {
                return true;
            }
        ...
        private void unityToPBRMaterial(Material mat, ref GlTF_Material material)
        {
            ...
            else
            {
                ...
                isMetal = mat.shader.name == "GLTFUtility/Standard Transparent (Metallic)" || mat.shader.name == "GLTFUtility/Standard (Metallic)" || mat.shader.name == "Standard";
                GlTF_Writer.hasSpecularMaterials = GlTF_Writer.hasSpecularMaterials || !isMetal;
                material.isMetal = isMetal;
                isStandard = mat.shader.name == "Standard";
                ...
                //Check transparency
                hasTransparency = handleTransparency(ref mat, ref material);//mat.shader.name == "GLTFUtility/Standard Transparent (Metallic)"; //handleTransparency(ref mat, ref material);
            ...
            if (mat.HasProperty("_MainTex") && mat.GetTexture("_MainTex") != null)
            {
                ...
                int baseColorTexture = processTexture((Texture2D)mat.GetTexture("_MainTex"), hasTransparency ? IMAGETYPE.RGBA_OPAQUE : IMAGETYPE.RGBA);
                textureValue.intValue.Add("index", baseColorTexture);
            ...
            if (isMaterialPBR)
            {
                if (isMetal)
                {
                    ...
                    if(isStandard)
                    {
                        ...
                        roughnessFactor.value = hasPBRMap ? 1.0f : mat.GetFloat("_Glossiness"); 
                    ...
                    else
                    {
                        var roughnessFactor = new GlTF_Material.FloatValue();
                        roughnessFactor.name = "roughnessFactor";
                        roughnessFactor.value = hasPBRMap ? 1.0f : mat.GetFloat("_Roughness"); 
                        material.pbrValues.Add(roughnessFactor);
                    }

                    if(mat.HasFloat("_AlphaCutoff") && mat.GetFloat("_AlphaCutoff") != 0)
                    {

                        var alphaCutoff = new GlTF_Material.FloatValue();
                        alphaCutoff.name = "alphaCutoff";
                        alphaCutoff.value = mat.GetFloat("_AlphaCutoff");//mat.GetFloat("_AlphaCutoff") != null ？ ;
                        string mode = "MASK";
                        GlTF_Material.StringValue alphaMode = new GlTF_Material.StringValue();
                        alphaMode.name = "alphaMode";
                        alphaMode.value = mode;
                    ...
                    if(mat.HasFloat(" _Cutoff") && mat.GetFloat(" _Cutoff") != 0)
                    {
                        var alphaCutoff = new GlTF_Material.FloatValue();
                        alphaCutoff.name = "alphaCutoff";
                        alphaCutoff.value = mat.GetFloat("_Cutoff");//mat.GetFloat("_AlphaCutoff") != null ？ ;
                        string mode = "MASK";
                        GlTF_Material.StringValue alphaMode = new GlTF_Material.StringValue();
                        alphaMode.name = "alphaMode";
                        alphaMode.value = mode;
        ...
        public string convertTexture(ref Texture2D inputTexture, string pathInProject, string exportDirectory, IMAGETYPE format)
        {
            ...
            string outputFilename = inputTexture.name + (format == IMAGETYPE.RGBA_OPAQUE ? ".png" : ".jpeg");//Path.GetFileNameWithoutExtension(pathInProject) + (format == IMAGETYPE.RGBA ? ".png" : ".jpg");
            string exportPath = exportDir + "/" + outputFilename;  // relative path inside the .zip
            string pathInGltfFile = pathInArchive + "/" + outputFilename;
            File.WriteAllBytes(exportPath, (format == IMAGETYPE.RGBA || format == IMAGETYPE.NORMAL_MAP ? newtex.EncodeToJPG() : newtex.EncodeToPNG()));
            //string outputFilename = inputTexture.name + (format == IMAGETYPE.RGBA_OPAQUE ? ".png" : ".png");//Path.GetFileNameWithoutExtension(pathInProject) + (format == IMAGETYPE.RGBA ? ".png" : ".jpg");
            //string exportPath = exportDir + "/" + outputFilename;  // relative path inside the .zip
            //string pathInGltfFile = pathInArchive + "/" + outputFilename;
            //File.WriteAllBytes(exportPath, (format == IMAGETYPE.RGBA ? newtex.EncodeToPNG() : newtex.EncodeToJPG(85)));
