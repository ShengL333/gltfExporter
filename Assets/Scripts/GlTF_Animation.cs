...
namespace Uinty2glTF
{
    public class GlTF_Animation : GlTF_Writer
    {
        ...
        public void Populate(AnimationClip clip, Transform tr, bool bake = true)
        {
            ...
            if (bake)
            {
                ...
                foreach (string target in targetCurvesBinding.Keys)
                {
                    ...
                    targetWeight.id = GlTF_Node.GetNameFromObject(targetObject);
                    ...
                    GlTF_Accessor weightAccessor = new GlTF_Accessor(targetId + "_WeightAccessor_" + clip.name, GlTF_Accessor.Type.SCALAR, GlTF_Accessor.ComponentType.FLOAT);
                    scaleAccessor.bufferView = GlTF_Writer.floatBufferView;
                    ...
                    float[] weight = null;
                    bakeCurveSet(targetCurvesBinding[target], clip.length, bakingFramerate, ref times, ref positions, ref rotations, ref scales, ref weight);
                    ...
                    //weightAccessor.Populate(weight);
                }
            }
            else
            {
        ...
        private void bakeCurveSet(TargetCurveSet curveSet, float length, int bakingFramerate, ref float[] times, ref Vector3[] positions, ref Vector4[] rotations, ref Vector3[] scales, ref float[] weight)
        {
